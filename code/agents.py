...
try:
    ...
    def _missing_sglang(*_args, **_kwargs):
        raise RuntimeError(
            "sglang is required for a separate local code-model endpoint."
        )

    assistant = gen = user = _missing_sglang

all_input_token, all_output_token = 0, 0


def _console_safe_text(value, encoding=None):
    target_encoding = encoding or getattr(sys.stdout, "encoding", None) or "utf-8"
    return str(value).encode(target_encoding, errors="replace").decode(target_encoding)

VOTE_PROMPT_AS = (
    "Given a question, a table, past reasonings and several current intermediate "
    "reasoning paths, decide which current reasoning path is the most promising "
    "in terms of solving the question. Analyze each path in detail, then conclude "
    'in the last line "The best path is {s}", where s is the integer id of the path.\n'
)
...
def _load_docstore():
    try:
        ...
        return None
    return DocstoreExplorer(Wikipedia())
...
class ReactAgent:
    ...
    def __init__(self,
        ...
        self.client = getattr(self.api_backend, "client", None)
        ...
        self.pre_ans_all = []
        self.docstore = None if without_tool else _load_docstore()
    ...
    def _generate_candidates(self, prompt, model_name, count):
    ...
    def retriever_tool(self, instruction):
        ...
        if self.code_model_name == self.plan_model_name:
            ...
            if self.api_backend is not None:
                ...
                code_strings = self._generate_candidates(
                    prompt,
                    model_name=self.code_model_name,
                    count=max_attempt,
                )
        ...
        if len(thoughts) > 0:
            ...
            if self.api_backend is not None:
                outputs = self._generate_candidates(
                    VOTE_PROMPT_AS + all_paths,
                    model_name=self.plan_model_name,
                    count=1,
                )[0]
            else:
                from tot import llm_reward, vote_prompt_as

                outputs, _, _ = llm_reward(
                    reasoning_paths=all_paths,
                    vote_prompt=vote_prompt_as,
                    model_type="open",
                    model_name=self.plan_model_name,
    ...
    def step(self) -> None:
        ...
        if self.direct_reasoning:  # 判断是否处于直接推理模式
            ...
            if self.api_backend is not None:
                code_sampled = self._generate_candidates(
                    prompt,
                    model_name=self.code_model_name,
                    count=self.code_sample,
                )
            else:
            ...
            if self.use_pre_answer and self.pre_ans:  # 判断是否已有预答案
                ...
                print(_console_safe_text(self.scratchpad))  # 输出当前 scratchpad

    def prompt_agent_gpt(self) -> str:
        """Send the current REACT prompt to the configured API backend."""
        prompt = self._build_agent_prompt()
        return self._generate_candidates(
            prompt,
            model_name=self.plan_model_name,
            count=self.plan_sample,
        )

    def prompt_agent_gpt_coder(self, prompt) -> str:
        """Generate code candidates through the configured model backend."""
        return self._generate_candidates(
            prompt,
            model_name=self.code_model_name,
            count=self.code_sample,
    ...
    def get_quick_answer(self):
        ...
        answer = self._generate_candidates(
            prompt,
            model_name=self.plan_model_name,
            count=self.plan_sample,
        )
    ...
    def prompt_agent(self, mode="both") -> str:
        ...
        if self.api_backend is not None:
            if self.as_reward in {"logp", "combined"}:
                raise RuntimeError(
                    f"as_reward={self.as_reward!r} requires a local model backend."
                )
            return self._generate_candidates(
                prompt,
                model_name=self.plan_model_name,
